...
| `Driver::writeEepromPage` | `test_eeprom_page_positions_lengths_and_frames_are_exact`; `test_write_validation_is_complete_transactional_and_callback_free`; `test_eeprom_write_public_api_transport_matrix_tracks_once`; `test_write_nacks_map_every_address_and_data_phase_without_replay`; `test_write_transport_failures_preserve_phase_prefix_and_exact_error` | page write/effect mapping, including malformed evidence |
...
| `Driver::permanentlyLockSecurity` | `test_lock_mutation_already_verified_mismatch_and_hold_ambiguity`; `test_lock_postcheck_failure_preserves_accepted_and_exact_status`; `test_lock_and_freeze_mutation_nack_phases_are_exact`; `test_security_public_api_transport_matrix_tracks_once` | lock precheck/mutation/postcheck |
